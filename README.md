...
- Scroll the mouse wheel (or drag vertically on the map on touch devices) to shift every clock together in 10-minute steps; the day/night shading moves with it
- Settings panel for adding and removing locations
- Your selected clocks are saved to the browser's localStorage and restored on your next visit
...
- Scroll the mouse wheel, or drag vertically on the map, to move every clock by 10-minute steps (day/night shading moves too)
- Horizontal drags are reserved for scrolling the clock list, so they don't affect the time
- Open **Settings** (top right) to remove clocks or add one by searching for a city name
